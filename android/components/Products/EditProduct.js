...
function EditProduct({route}){
    ...
    const [namProduct, setNamaProduct] = useState(param.nama_product)
    const [sku, setSKU] = useState(param.sku)
    const [stock, setStock]= useState(param.stock)
    const [harga, setHarga]= useState(param.harga)
    const [deskripsi, setDeskripsi]= useState(param.desc)
    ...
    const saveProductHandler = async() => {
        ...
        let res = await fetch(
            `${SERVER_HOST}/api/product/update/${param.id}`,
            {
              method: 'post',
              body: formData,
              headers: {
            ...
            <Modal isOpen={showModal} onClose={() => setShowModal(false)} size="xl">
                <Modal.Content maxWidth="350">
                    ...
                    <Modal.Body>
                        <View>
                            <Text fontSize={20}>
                                {isSaved ? 'Produk berhasil di update' :'Gagal mengupdate produk'}
                            </Text>
                        </View>
                    </Modal.Body>
                    <Modal.Footer>
                ...
                <ScrollView style={[Style.container, {backgroundColor:'white'}]}>
                    <View mb={5}>
                        ...
                        <Input  fontSize={16}
                                ...
                                defaultValue={param.nama_product}
                    ...
                    <View mb={5}>
                        ...
                        <Input fontSize={16}
                                placeholder="Stock"
                                defaultValue={String(param.stock)}
                    ...
                    <View mb={5}>
                        ...
                        <Input fontSize={16}
                                placeholder="Harga"
                                defaultValue={String(param.harga)}
                    ...
                    <View mb={5}>
                        ...
                        <TextArea h={20} placeholder="Deskripsi" w="100%" maxW="500" defaultValue={param.desc} onChangeText={(text) => setDeskripsi(text)} />
                    </View>
                    <View mb={5}>
                        {
                            loading ? (
